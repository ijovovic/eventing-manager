...
test: envtest	## Run tests.
	GOTOOLCHAIN=go1.27.0+auto KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test ./... -coverprofile cover.out
...
GOLANG_CI_LINT_VERSION ?= v2.9.0 ## Keep this the same as in .github/workflows/lint-go.yml
